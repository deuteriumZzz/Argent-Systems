...
### 5. Hedge-плечо, капированный risk-parity и funding-стресс-тест

Раздел 2 показал, что risk-parity отдаёт carry почти весь вес (93.7% в
`results/risk_summary.csv`) просто потому что у него ниже волатильность —
не потому что остальные ноги бесполезны. Две вещи, добавленные отдельно
от основного набора из 132 стратегий:

- **`hedge_trend_vol_bear`** — явный шорт-хедж (шорт только при
  подтверждённом даунтренде + всплеске реализованной волатильности, иначе
  flat; никогда не лонг). Мотивация: у всех 132 стратегий один и тот же
  провальный режим (bear_2018/bear_2022/correction_2021 — см. раздел
  "Текущий набор"), потому что почти все они long-biased. Корреляция с
  остальными ногами отрицательная (-0.10…-0.21) — диверсифицирует не на
  бумаге, а по факту.
- **`backtest/build_hedged_portfolio.py`** — тот же risk-parity, что и в
  разделе 2, но с потолком 35% на ногу (чтобы carry не забирал всё) и
  добавленной hedge-ногой. Walk-forward split на 2023-01-01 (как в
  `tune_composite.py`): train Sharpe 0.68 → test (out-of-sample) Sharpe
  1.41 — не провалился на OOS, в отличие от v1-композита, потому что веса
  не фитились на прошлых доходностях (никакого grid search, чисто
  структурное inverse-vol взвешивание).
- **`backtest/stress_test_carry.py`** — прямой ответ на "хвостовой риск,
  который не виден в этих же данных" из раздела 3: инжектирует
  просадку funding-ставки в 14 дней по -0.8%/8ч (~6.7x хуже худшего
  реального принта в истории с 2020) и пересчитывает и carry-ногу
  отдельно, и весь капированный портфель. Результат
  (`results/carry_stress_test.csv`): сольная carry-нога теряет Sharpe
  10.6 → 1.84 и просаживается на -28.6% за окно шока — но у **капированного
  портфеля** максимальная просадка за всю историю не меняется вообще
  (-41.24% что до, что после шока), потому что 35%-потолок не даёт carry
  утащить весь портфель за собой; в само окно шока портфель всё равно
  уходит в -5.17% против +0.91% без шока — потолок ограничивает
  катастрофу, но не делает её бесплатной.

**Статус**: это по-прежнему чисто исследовательский/бэктест-пайплайн — ни
один из файлов в этом репозитории не подключён к исполнению ордеров на
бирже. Живая торговля (BitbotBY, отдельный репозиторий) использует
собственный набор стратегий и пока не содержит ни hedge-ноги, ни
funding-carry сделки, найденных здесь — перенос идей между проектами не
делался.

**Главный вывод всей сессии**: ни одна из 132 стратегий поодиночке пока не
доказала строгую статистическую значимость edge. Собственные
composite-попытки (`custom_regime_adaptive_composite` v1/v2) не
превзошли свои же компоненты, но капированный risk-parity с явной
hedge-ногой (раздел 5) держится out-of-sample лучше, чем любая из них по
отдельности — с оговоркой, что часть теста (2023-2025) пришлась на
бычий период для BTC, и что реальный риск контрагента/биржи для carry
(в отличие от риска funding-ставки, уже застресс-тестированного) в
бэктесте всё ещё не моделируется. Дальнейшие шаги: тестировать на
действительно новых данных (не повторно использовать одну и ту же
историю BTC), смоделировать риск контрагента/маржи для carry отдельно от
funding-риска, и/или подключить принципиально другие источники данных
(ончейн для других активов, ордербук, мультиактивная кросс-секционная
ротация), которые эта поправка на количество попыток пока не затрагивала.
